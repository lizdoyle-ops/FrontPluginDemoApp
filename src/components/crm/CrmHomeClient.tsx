...
export function CrmHomeClient() {
  ...
  useEffect(() => {
    if (!tab.startsWith("custom:")) return;
    const listId = tab.slice("custom:".length);
    if (!cfg.customObjectDefinitions.some((d) => d.id === listId)) {
      setTab("account");
    }
  }, [tab, cfg.customObjectDefinitions]);
  ...
  const crmTabs = useMemo(() => {
    const customTabs = cfg.customObjectDefinitions.map((d) => ({
      id: `custom:${d.id}` as TabId,
      label: d.title,
    }));
  ...
  const tabPanel = () => {
    ...
    if (tab.startsWith("custom:")) {
      const listId = tab.slice("custom:".length);
      const d = cfg.customObjectDefinitions.find((x) => x.id === listId);
      if (!d) {
        return (
          <p className="text-[13px] text-zinc-500">
            This custom object is not defined. Add it in Admin centre → Custom
            objects.
          </p>
        );
      }
