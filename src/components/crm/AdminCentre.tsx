...
/** Every `SectionId` appears once; append new ids if storage predates them. */
function mergeNativeSectionOrder(stored: SectionId[]): SectionId[] {
  const allowed = new Set<string>(SECTION_IDS);
  const filtered = stored.filter((id) => allowed.has(id));
  const seen = new Set<SectionId>();
  const base: SectionId[] = [];
  const seed = filtered.length > 0 ? filtered : [...DEFAULT_SECTION_ORDER];
...
export function AdminCentre({
  ...
  const nativeSectionOrder = useMemo(
    () => mergeNativeSectionOrder(cfg.sectionOrder),
    [cfg.sectionOrder],
  );
  ...
  const onSectionDragEnd = (e: DragEndEvent) => {
    ...
    const order = mergeNativeSectionOrder(cfg.sectionOrder);
  ...
  const saveNewCustomObject = () => {
    ...
    notify(
      ok
        ? "Object saved. It appears under Plugin dashboard lists (toggle Show on plugin) and as a Back Office tab; API docs list CRUD for its listId."
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-6 lg:px-0">
      ...
      <Card className="space-y-3 p-5">
        ...
        <p className="text-[12px] text-zinc-500">
          ...
          its own tab under Back Office → Accounts when you open a contact.
        </p>
        ...
        >
          <SortableContext
            items={[...nativeSectionOrder]}
            strategy={verticalListSortingStrategy}
          >
            <div className="space-y-1.5">
              {nativeSectionOrder.map((id) => (
