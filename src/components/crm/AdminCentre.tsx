...
export function AdminCentre() {
  ...
  const addCustomObject = () => {
    ...
    const next: CustomObjectDefinition[] = [
      ...cfg.customObjectDefinitions,
      { id, title, fieldKeys: ["name", "status"] },
    ];
    cfg.setCustomObjectDefinitions(next);
    cfg.setCustomObjectOrder((o) => [...o, id]);
    cfg.setVisibleCustomObjects((v) => ({ ...v, [id]: true }));
    setNewObjectTitle("");
  };
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 py-6 lg:px-0">
      ...
      <Card className="space-y-3 p-5">
        ...
        <p className="text-[12px] text-zinc-500">
          Drag to reorder, toggle visibility. Matches what appears in the Front
          plugin for matching contacts.
        </p>
        <DndContext
          sensors={sensors}
      ...
      <Card className="space-y-4 p-5">
        ...
        <p className="text-[12px] text-zinc-500">
          Define extra list types with their own fields. They appear on the
          plugin after standard sections (same show/reorder pattern). Edit rows
          per contact on the CRM → Custom lists tab.
        </p>
        <DndContext
          sensors={sensors}
          collisionDetection={closestCenter}
        ...
        <div className="flex flex-wrap gap-2 rounded-lg border border-dashed border-zinc-200 p-3">
          <input
            className="min-w-[12rem] flex-1 rounded border border-zinc-200 px-2 py-1.5 text-[13px]"
            placeholder="New object title"
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-lg bg-[var(--brand-color)] px-3 py-2 text-[12px] font-semibold text-white"
          >
